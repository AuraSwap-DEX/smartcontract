...
interface IMasterChefAura {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this poolInfo. AURA to distribute per block.
        uint256 lastRewardTimestamp; // Last block timestamp that AURA distribution occurs.
        uint256 accAuraPerShare; // Accumulated AURA per share, times 1e12. See below.
    }
...
/**
 * This is a sample contract to be used in the MasterChefAura contract for partners to reward
 * stakers with their native token alongside AURA.
 *
 * It assumes no minting rights, so requires a set amount of YOUR_TOKEN to be transferred to this contract prior.
 * E.g. say you've allocated 100,000 XYZ to the AURA-XYZ farm over 30 days. Then you would need to transfer
...
contract SimpleRewarderPerSec is IRewarder, Ownable, ReentrancyGuard {
    ...
    /// @notice Info of each MCA poolInfo.
    ...
    IMasterChefAura public immutable MCA;
    ...
    modifier onlyMCA() {
        require(msg.sender == address(MCA), "onlyMCA: only MasterChefAura can call this function");
        _;
    }
    ...
    /// @notice Function called by MasterChefAura whenever staker claims AURA harvest. Allows staker to also receive a 2nd reward token.
    /// @param _user Address of user
    /// @param _lpAmount Number of LP tokens the user has
    function onAuraReward(address _user, uint256 _lpAmount) external override onlyMCA nonReentrant {
        updatePool();
